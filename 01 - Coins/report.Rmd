...
teste.t.value
```
O teste de hipóteses para dois casos sugere rejeitar a hipótese nula, considerando-se a estatística $t$ de `r round(teste.t.coins$statistic, digits=2)` ($|t_{0}|>0.05$ para `r teste.t.coins$parameter` graus de liberdade), no caso $A$, e `r round(teste.t.value$statistic,2)` ($|t_{0}|>0.05$ para `r teste.t.value$parameter` graus de liberdade) no caso $B$. 
...
Como desejávamos aumentar a potência do teste para o valor desejado de `r round(v_coins.desiredPower,2)`, observou-se, no caso $A$, que seria necessário um tamanho amostral de `r ceiling(v_coins.n)` e, para o caso $B$, `r ceiling(v_value.n)`, o que não é possível alcançar-se, já que a amostra única possui tamanho fixo. 
