...
```{r, include=TRUE, results='show',warning=FALSE,echo=TRUE}
teste.t.coins<-t.test(v_coins, mu=130)
teste.t.value<-t.test(v_value, mu=9.10)
teste.t.coins
teste.t.value
```
O teste de hipóteses para dois casos sugere rejeitar a hipótese nula, considerando que $|t_{0}|>0.05$ para `r teste.t.coins$parameter` graus de liberdade.

No entanto, considerando-se que o teste sugere a rejeição da hipótese nula, e considerando-se a potência desejada e o menor efeito de significância prática para os dois casos ($\delta^{\star}_A=10$ moedas e $\delta^{\star}_B=0.50$ centavos), realzou-se um teste de potência:
